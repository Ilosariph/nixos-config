...
let
  ...
  # Convert Hyprland-style monitor config to niri format
  # Example: "DP-3, 3440x1440@120, 0x0, 1" -> { name = "DP-3"; mode = "3440x1440@120"; position = { x = 0; y = 0; }; scale = 1.0; }
  ...
  # Map keyboard layout to niri bindings
  left = cfg.left;
  right = cfg.right;
  up = cfg.up;
  down = cfg.down;

  # Convert direction keys to niri keycodes
  keyToNiri = key:
  ...
  mouseAccelProfile = if cfg.accel_profile != null
                      then if lib.hasPrefix "custom" cfg.accel_profile
                           then "adaptive"  # niri doesn't support custom profiles
                           else cfg.accel_profile
                      else null;

  # Generate monitor output configurations
  ...
  # Base spawn commands based on statusbar choice
