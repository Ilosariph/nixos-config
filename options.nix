    ...
    programs = {
      ...
      evilAp = {
        enable = lib.mkOption {
          ...
          default = false;
          description = ''
            Enable the evil captive-portal access point. Broadcasts a bait WiFi
            network and serves a troll page to anyone who connects instead of
            giving them actual internet. Requires a wireless card that supports
            AP mode (check with: iw list | grep "AP").
          '';
        };
        ssid = lib.mkOption {
          type = lib.types.str;
          default = "Free_Airport_WiFi";
          description = "SSID (network name) to broadcast. Make it tempting.";
        };
        interface = lib.mkOption {
          type = lib.types.str;
          default = "wlan0";
          description = "Wireless interface to use as the access point.";
        };
        channel = lib.mkOption {
          type = lib.types.int;
          default = 6;
          description = "WiFi channel to broadcast on (2.4 GHz channels 1–13).";
        };
        portalHtml = lib.mkOption {
          type = lib.types.str;
          default = ''
            <!DOCTYPE html>
            <html lang="en">
            <head>
              ...
              <style>
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                h1 { font-size: 2.8rem; color: #ff4444; margin-bottom: 0.4rem; }
                .sub { color: #888; font-size: 0.85rem; margin-bottom: 2rem; letter-spacing: 0.1em; text-transform: uppercase; }
                p { line-height: 1.7; margin-bottom: 1rem; font-size: 1rem; }
                .highlight { color: #ffcc00; font-weight: bold; }
                .dim { color: #555; font-size: 0.8rem; margin-top: 2rem; }
            ...
            <body>
              <div class="card">
                <h1>womp womp</h1>
                <p class="sub">free wifi portal — authentication required</p>
                <p>congratulations. you have connected to a <span class="highlight">laptop</span>.</p>
                <p>there is no internet here. there never was.</p>
                <p>the access point is named <span class="highlight">Free_Airport_WiFi</span> because that's funny.</p>
                <p>you are currently being served this page by nginx, running on nixos,<br>
                   from someone's bag within roughly 15 metres of you.</p>
                <p class="dim">this machine is not responsible for any existential crises caused by this discovery<span class="blink">_</span></p>
              </div>
            </body>
            </html>
          '';
          description = "Full HTML content of the captive portal troll page.";
        };
      };
    };
    audio = {
